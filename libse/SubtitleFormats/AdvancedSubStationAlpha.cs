...
Format: Layer, Start, End, Style, Actor, MarginL, MarginR, MarginV, Effect, Text";
                ...
                foreach (var ssaStyle in GetSsaStyle(subtitle.Header))
                {
                    try
                    {
                        if (ssaStyle != null)
                        {
                            ...
                            ttStyles.AppendLine(string.Format(styleFormat, ssaStyle.Name, ssaStyle.FontName, ssaStyle.FontSize, GetSsaColorString(ssaStyle.Primary),
                                GetSsaColorString(ssaStyle.Secondary),
                    ...
                    if (name != null)
                    {
                        ...
                        try
                        {
                            if (color.StartsWith("rgb(", StringComparison.Ordinal))
                            {
                                ...
                                c = CustomColor.FromRgb(int.Parse(arr[0]), int.Parse(arr[1]), int.Parse(arr[2]));
                            }
                            else
                            {
        ...
        public static CustomColor GetSsaColor(string f, CustomColor defaultColor)
        {
            //Red = &H0000FF&
            //Green = &H00FF00&
            //Blue = &HFF0000&
            ...
            if (s.StartsWith('h') && s.Length == 7)
            {
                ...
                try
                {
                    return ColorTranslator.FromHex(s.Substring(4, 2), s.Substring(2, 2), s.Substring(0, 2));
                }
                catch
                {
                    return defaultColor;
            ...
            if (s.StartsWith('h') && s.Length == 9)
            {
                ...
                try
                {
                    var c = ColorTranslator.FromHex(s.Substring(4, 2), s.Substring(2, 2), s.Substring(0, 2));
                    return new CustomColor(c, alpha);
                }
                catch
                {
                    return defaultColor;
            ...
            uint number;
            if (uint.TryParse(f, out number))
            {
                var temp = new CustomColor(number);
                return new CustomColor(temp, 255);
            }
        ...
        public static string GetSsaColorString(CustomColor c)
        {
                ...
                else if (s.Replace(" ", string.Empty).StartsWith("style:", StringComparison.Ordinal))
                {
                    if (line.Length > 10)
                    {
                        ...
                        for (int i = 0; i < format.Length; i++)
                        {
                            ...
                            {
                                style.Primary = GetSsaColor(f, CustomColor.White);
                            }
                            else if (i == secondaryColourIndex)
                            {
                                style.Secondary = GetSsaColor(f, CustomColor.Yellow);
                            }
                            else if (i == tertiaryColourIndex)
                            {
                                style.Tertiary = GetSsaColor(f, CustomColor.Yellow);
                            }
                            else if (i == outlineColourIndex)
                            {
                                style.Outline = GetSsaColor(f, CustomColor.Black);
                            }
                            else if (i == backColourIndex)
                            {
                                style.Background = GetSsaColor(f, CustomColor.Black);
                            }
                            else if (i == boldIndex)
                            {
                                style.Bold = f == "-1" || f == "1";
